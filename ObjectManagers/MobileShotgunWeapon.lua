	...
	elseif not waiting and fireType == "FullAuto" then
		...
		coroutine.wrap(function()
			for i =0, numShots do
				...
				local bullet  = replicatedStorage.BulletStorage:FindFirstChild("Pellet"):Clone()
				local flash   = shotgunFlash:Clone()
				bullet.Parent = game.Workspace.CurrentCamera
				local x = math.random(-spread * 100, spread * 100) / 100
				local y = math.random(-spread * 100, spread * 100) / 200
				bullet.CFrame = CFrame.new(center.Position, endPos) * CFrame.Angles(math.rad(y), math.rad(x), 0)
