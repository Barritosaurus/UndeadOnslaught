...
function WeaponEquipped()
	...
	lookingPart    = game.Workspace.Camera:WaitForChild("LookingPart")
	FireSound      = handle.FireSound
	ReloadSound    = handle.Reload
	OutOfAmmoSound = handle.OutOfAmmo
	EquipAnimation()
	currentGUI = player.PlayerGui.weaponGUI
	mobileControlGUI = player.PlayerGui:WaitForChild("MobileControlGUI")
	setGUI()
	reloadButton          = mobileControlGUI:WaitForChild("ReloadButton")
	...
	equipped = true
	shooting = mobileControlGUI:WaitForChild("CurrentlyShooting")

	reloadConnection = reloadButton.InputBegan:Connect(function()
		onReload()
	end)

	renderStepConnection = runService.RenderStepped:Connect(function()
		if shooting.Value and equipped == true and reloading == false and humanoid ~= nil then
			shoot()
		end
	end)
end
...
function shoot()
	...
	elseif not waiting and fireType == "FullAuto" then
		...
		coroutine.wrap(function()
			...
			local endPos  = Vector3.new(lookingPart.Position.X, center.Position.Y, lookingPart.Position.Z)
			local bullet  = replicatedStorage.BulletStorage:FindFirstChild("Energy"):Clone()
			local flash   = plasmaFlash:Clone()
			...
			connection = game:GetService("RunService").Heartbeat:Connect(function()
				if penetration == true then
					...
					if hitPart and hitPart.Parent ~= workspace and hitPart.Parent ~= game.Workspace.CurrentCamera and hitPart.Parent.Name ~= player.Name and hitPart.Name ~= "Handle" and hitPart.Name ~= "NonTarget" and hitPart.Parent.Name ~= "Handle" and not CollectionService:HasTag(hitPart.Parent, "Player") then
						coroutine.wrap(function()
							onHit(hitPart, hitPos, center.Position, player, damage, cashMultiplier.Value)
							--onHitEvent:FireServer(hitPart, hitPos, center.Position, player, damage, cashMultiplier.Value)
						end)()
						bullet:Destroy()
					end
				end
			...
			GunEvent:FireServer(center.Position + (humanoidRootPart.Velocity / 12), lookingPart.Position, FireSound, weaponType, penetration, penValue, bulletSpeed, spread, damage, numShots, handle.Emitter, DEBUG)
			ShootAnimation()
		end)()
		local t = tick()
		while (tick() - t) < fireRate do
	...
	elseif not waiting and fireType == "SemiAuto" then
		...
		coroutine.wrap(function()
			local x = math.random(-spread * 100, spread * 100) / 100
			local endPos  = Vector3.new(lookingPart.Position.X, center.Position.Y, lookingPart.Position.Z)
			local bullet  = replicatedStorage.BulletStorage:FindFirstChild("Plasma"):Clone()
			local flash   = plasmaFlash:Clone()
			bullet.Parent = game.Workspace.CurrentCamera
			bullet.CFrame = CFrame.new(center.Position, endPos) * CFrame.Angles(0, math.rad(x), 0)
			local currentPen = bullet.pen
		...
		for i = 0, numShots do
			if ammo.Value > 0 and equipped == true then
				...
				coroutine.wrap(function()
					...
					Debris:AddItem(bullet, 1)
					GunEvent:FireServer(center.Position + (humanoidRootPart.Velocity / 8), lookingPart.Position, FireSound, weaponType, penetration, penValue, bulletSpeed, spread, damage, numShots, handle.Emitter, DEBUG)
					ShootAnimation()
				end)()
				local q = tick()
