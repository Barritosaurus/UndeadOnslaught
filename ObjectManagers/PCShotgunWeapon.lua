...
function WeaponReload()
	if reloading == false and equipped == true and ammo.Value < maxAmmo and shooting == false then
		reloading = true
		...
		while not breaking do
			...
			if equipped == false or ammo.Value > maxAmmo - 1 or shooting == true then
				ReloadSound:Stop()
				reloadAnim:Stop()
				reloading = false
				breaking = true
...
function shoot()
	...
	elseif not waiting and fireType == "FullAuto" then
		...
		coroutine.wrap(function()
			...
			GunEvent:FireServer(center.Position + (humanoidRootPart.Velocity / 8), mouse.Hit.p, FireSound, weaponType, penetration, penValue, bulletSpeed, spread, damage, numShots, handle.Emitter, DEBUG)
			ShootAnimation()
		end)()
		local t = tick()
		while (tick() - t) < fireRate do
